...
c
         ...
         enddo
         do id=1,ND
            write(*,*) id,deltaT(id)/(3.*T(id)),deltaH(id)/Htot
         enddo
c        Verifier convergence Temp
         ...
         ktemp=k-kble
         kble=kble+1
         write(*,*) 'Temperature convergee iteration',ktemp
c        Verifier convergence Flux
         do id=1,ND
            if (abs(deltaH(id)/Htot).gt.1.e-3) goto 501
         enddo
         kflux=k
         write(*,*) 'Flux converge iteration',kflux
         goto 502
...
c     probleme1
      ...
      do id=1,ND
         write(1,*) tau(id),T(id),P(id)
      enddo
      do id=1,ND
         write(1,*) deltaT(id)/(3.*T(id)),deltaH(id)/Htot
      enddo
      close(1)
c     probleme2
      open(2,file='dev5prob2.txt',status='old')
      do id=1,ND
         write(2,*) tau(id),T(id),Tgris(id)
      enddo
      close(2)
c     probleme3
      open(3,file='dev5prob3.txt',status='old')
      do k=1,kflux
         do id=1,ND
            write(3,*) tau(id),deltaTsT(k,id)
         enddo
      enddo
      close(3)
c     probleme4
      open(4,file='dev5prob4.txt',status='old')
      do k=1,kflux
         do id=1,ND
            write(4,*) tau(id),deltaHsH(k,id)
         enddo
      enddo
      close(4)
c     probeme5
      open(5,file='dev5prob5.txt',status='old')
      do k=1,kflux
         do id=1,ND
            write(5,*) tau(id),condeq(k,id)
         enddo
      enddo
      close(5)
c
